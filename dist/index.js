...
// src/services/mockupRepository.ts
...
class MockupRepository {
  ...
  getVariantEndpoints(variant) {
    const variantRoot = import_path.default.join(this.rootDir, variant);
    if (!import_fs.default.existsSync(variantRoot))
      return [];
    const endpoints = [];
    const walk = (currentPath) => {
      const entries = import_fs.default.readdirSync(currentPath, { withFileTypes: true });
      entries.forEach((entry) => {
        const entryPath = import_path.default.join(currentPath, entry.name);
        ...
        if (entry.name !== "index.mock")
          return;
        const relativePath = import_path.default.relative(variantRoot, entryPath).split(import_path.default.sep).join("/");
  ...
  ensureMockupForEndpoint(endpoint, variant, responseObject) {
    ...
    if (!import_fs.default.existsSync(filePath)) {
      import_fs.default.writeFileSync(filePath, JSON.stringify(responseObject, null, 2), "utf-8");
    }
  }
  endpointToMockupPath(endpoint, variant) {
    ...
    if (segments.length === 0) {
      return import_path.default.join(this.rootDir, variant, "index.mock");
    }
    return import_path.default.join(this.rootDir, variant, ...segments, "index.mock");
  }
...
// src/services/pathClassifier.ts
class PathClassifier {
  ...
  static isKnownPath(inputPath, options) {
    ...
    const isKnown = options.knownPaths.includes(normalizedPath) || options.knownPatterns.some((pattern) => pattern.test(normalizedPath)) || options.knownApiPaths.includes(normalizedPath) || options.knownApiPatterns.some((pattern) => pattern.test(normalizedPath));
    return {
      isKnown,
...
class TrafficService {
  ...
  constructor(projectRoot) {
    ...
    this.trafficFilePath = import_path2.default.join(projectRoot, "traffic.txt");
    this.botsFilePath = import_path2.default.join(projectRoot, "bots.txt");
    this.ensureLogFiles();
  }
  registerLoggingMiddleware(app, knownPathOptions) {
    app.use((req, _res, next) => {
      import_fs2.default.writeFileSync(this.botsFilePath, this.getAllBotsRequests(knownPathOptions));
      if (!req.originalUrl.match(/\.(js|css|ico|png|jpg|jpeg|gif|svg|webp)$/) && req.originalUrl !== "/support" && req.headers["user-agent"] !== "local-honeypot-tester") {
        ...
        import_fs2.default.appendFile(this.trafficFilePath, `${logLine}
`, (error) => {
          if (error)
            console.log(error);
        });
  ...
  getAllBotsRequests(knownPathOptions) {
    return import_fs2.default.readFileSync(this.trafficFilePath, "utf-8").split(`
`).filter((log) => {
...
`);
  ...
  ensureLogFiles() {
    if (!import_fs2.default.existsSync(this.trafficFilePath))
      import_fs2.default.writeFileSync(this.trafficFilePath, "");
    if (!import_fs2.default.existsSync(this.botsFilePath))
      import_fs2.default.writeFileSync(this.botsFilePath, "");
  }
}

// src/services/unhandledRoutesService.ts
class UnhandledRoutesService {
  ...
  getUnhandledRoutes(additionalEndpoints, knownPathOptions) {
    ...
    botsRequests.split(`
`).forEach((botRequest) => {
      const requestPath = botRequest.split(" - ")[2]?.replace(/^(GET|POST|DELETE|PUT|PATCH|HEAD) /, "").trim();
      if (!requestPath)
        return;
...
// src/index.ts
var __dirname = "/home/lsannier/express-honeypot-middleware/src";
...
module.exports = (app, options) => {
  ...
  const projectRoot = __dirname === "." ? process.cwd() : import_path3.default.resolve(__dirname, "..");
  const mockupRepository = new MockupRepository(import_path3.default.join(projectRoot, "mockups"));
...
`));
  ...
  app.get("*.php", async (req, res) => {
    ...
    const response = await import_node_fetch.default(`${protocol}://${host}${req.originalUrl.split(".php")[0]}`);
